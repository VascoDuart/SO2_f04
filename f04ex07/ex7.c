...
LPVOID CALLBACK comandos(LPVOID data, DWORD low, DWORD high) {
	TDados* td = (TDados*)data;
...
int _tmain(int argc, TCHAR* argv[]) {
	...
	liTempo.QuadPart = numseg * -10000000LL;
	SetWaitableTimer(hT, &liTempo, 0, comandos, (LPVOID)&tinfo, FALSE);
	...
	do {					//Thread preto (Ti)
		WaitForSingleObjectEx(hEv, INFINITE, TRUE);		//EVENTO de reset automático...
		_tprintf_s(_T("*"));
